...
def is_cell_responsive(
    n_responsive_trials, weighted_avg, min_responsive_trials=7, nstd=3
):
    """
    ...
    if (
        max_value_responsive_trials >= min_responsive_trials
        and max_value_responsive_trials
        >= mean_responsive_trials + nstd * std_responsive_trials
        and max_value_all_trials >= mean_all_trials + nstd * std_all_trials
    ):
        return True
    else:
        return False
...
def make_sample_spontanous(
    ...
    spont_start_end_times = np.where(
        np.logical_and(spont_start_time <= times, times <= spont_end_time)
    )[0]
    ...
    for s in range(sample_spontaneous):
        spont_vals[:, s] = dff[:, spont_idxs[s] : spont_idxs[s] + lsn_pres_len].mean(
            axis=1
        )
...
def make_sample_spontanous_ns(
    ...
    spont_idxs = rng.choice(spont_times, sample_spontaneous)
    for s in range(sample_spontaneous):
        spont_vals[:, s] = dff[:, spont_idxs[s] : spont_idxs[s] + ns_pres_len].mean(
            axis=1
        )
...
def calc_lsn_vals(
    ...
    tot_lsn_frames = 0
    lsn_start_times = lsn.stim_table.start
    lsn_end_times = lsn.stim_table.end
    for cell in range(n_cells):
        ...
        for t in range(n_tot_pres):
            ...
            lsn_start_time = lsn_start_times[t]
            lsn_end_time = lsn_end_times[t]
            lsn_start_end_idxs = np.where(
                np.logical_and(lsn_start_time <= times, times <= lsn_end_time)
            )[0]
            ...
            if len(lsn_start_end_idxs) == 0:
                ...
                lsn_val = dff[
                    cell,
                    onset_delay
                    + np.min(lsn_start_end_idxs) : onset_delay
                    + np.max(lsn_start_end_idxs)
                    + offset_delay,
                ].mean(
                    axis=0
                )  # interval selected: (3851, 3853)

            lsn_vals[t, cell] = np.maximum(lsn_val, 0)
            tot_lsn_frames += len(
                lsn_start_end_idxs
            )  # this is redundant and equal for every cell, fix later
...
def calc_ns12_vals(
    ...
    for cell in range(n_cells):
        ...
        for t in range(n_tot_pres):
            ...
            ns_start_end_idxs = np.where(
                np.logical_and(ns_start_time <= times, times <= ns_end_time)
            )[0]
            ...
            if len(ns_start_end_idxs) == 0:
                ...
                ns_val = dff[
                    cell,
                    onset_delay
                    + np.min(ns_start_end_idxs) : onset_delay
                    + np.max(ns_start_end_idxs)
                    + offset_delay,
                ].mean(axis=0)
...
def calc_pvals_for_plane_using_zscore_one_tailed(
    lsn,
    session,
    plane,
    lsn_vals,
    ...
    logging.info(
        f"{session.get_session_id()} ({session.get_plane_depth(plane)} µm): {np.median(traces.time.diff('time')):.4f}"
    )
    ...
    spont_vals = make_sample_spontanous(
        n_cells, lsn, times, dff, tot_lsn_frames, n_tot_pres, rng, sample_spontaneous
    )
...
def calc_ns12_pvals_for_plane_using_zscore_one_tailed(
    ns12,
    session,
    plane,
    ns12_vals,
    ...
    spont_vals = make_sample_spontanous_ns(
        n_cells, ns12, times, dff, tot_ns12_frames, n_tot_pres, rng, sample_spontaneous
    )
...
def calc_ns118_pvals_for_plane_using_zscore_one_tailed(
    ns118,
    session,
    plane,
    ns118_vals,
...
def rf_z_test(matrix, masked_z=True, square_mask_length: int = 7):
    """
    ...
    padded_mat = np.pad(
        matrix, pad_width=int(pad_wid), mode="constant", constant_values=np.nan
    )
    ...
    for mat_x in range(matrix.shape[0]):
        for mat_y in range(matrix.shape[1]):
            ...
            if masked_z == True:
                ...
                neighbors = padded_mat[
                    padded_mat_x - int(pad_wid) : padded_mat_x + int(pad_wid) + 1,
                    padded_mat_y - int(pad_wid) : padded_mat_y + int(pad_wid) + 1,
                ]
...
def find_rf_center(n_responsive_trials_events, weighted_avg_events):
    ...
    max_coord_nrt = np.unravel_index(
        np.argmax(n_responsive_trials_events), n_responsive_trials_events.shape
    )

    # Generating the neighboring coordinates of the max value in A
    neighbors_a = [
        (max_coord_nrt[0] + dx, max_coord_nrt[1] + dy)
        for dx in range(-1, 2)
        for dy in range(-1, 2)
    ...
    matching_coords = [
        coord for coord in coordinates_sig_z if tuple(coord) in neighbors_a
    ]
    ...
    row_range = range(
        max(0, rf_center_x - 2),
        min(n_responsive_trials_events.shape[0], rf_center_x + 3),
    )
    col_range = range(
        max(0, rf_center_y - 2),
        min(n_responsive_trials_events.shape[1], rf_center_y + 3),
    )
...
def assign_angle(data, center_x, center_y):
    ...
    if (center_y + 1 < cols and data[center_x, center_y + 1] != 0) or (
        center_y - 1 >= 0 and data[center_x, center_y - 1] != 0
    ):
        angle_degrees = 0
    elif (center_x + 1 < rows and data[center_x + 1, center_y] != 0) or (
        center_x - 1 >= 0 and data[center_x - 1, center_y] != 0
    ):
        angle_degrees = 90
    elif (
        center_y + 1 < cols
        and center_x + 1 < rows
        and data[center_x + 1, center_y + 1] != 0
    ) or (
        center_y - 1 >= 0
        and center_x - 1 >= 0
        and data[center_x - 1, center_y - 1] != 0
...
def find_rf_center_v2(n_responsive_trials_events, weighted_avg_events):
    ...
    # Define the range for rows and columns
    row_range = range(
        max(0, rf_center_x - 1), min(weighted_avg_events.shape[0], rf_center_x + 2)
    )
    col_range = range(
        max(0, rf_center_y - 1), min(weighted_avg_events.shape[1], rf_center_y + 2)
    )
...
def func(center_x, center_y, height, width_x, width_y, angle, f_type):
    ...
    if f_type == 1:
        ...
        return lambda x, y: height * torch.exp(
            ...
            * (
                (
                    (cos_theta * (x - center_x) + sin_theta * (y - center_y)) ** 2
                    / torch.abs(width_x)
                )
                + (
                    (sin_theta * (x - center_x) - cos_theta * (y - center_y)) ** 2
                    / torch.abs(width_y)
                )
            )
        )
        ...
        return lambda x, y: height / (
            0.1 + torch.abs(center_x - x) + torch.abs(center_y - y)
        )
...
def training_loop(
    ...
    for i in range(n_training):
        ...
        # Third implentation
        loss = (
            torch.sqrt(F.mse_loss(preds, y))
            + norm_weight * model.weights[3:].norm()
            + balance_weight * torch.squeeze(m(diff[None, :]))
        )
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
...
def centroid(
    ...
    if initialization == "default":
        weights = torch.tensor(
            [float(max_x), float(max_y), float(height), 1, 1, 0]
    ...
    if f_type == 6:
        ...
        if np.abs(max_x - 4) < 4 - shift + 1 and np.abs(max_y - 7) < 7 - shift + 1:
            data_crop = data[
                max_x - shift : max_x + shift, max_y - shift : max_y + shift
            ]
        else:
            shift = 0
            data_crop = data
        center_x_crop, center_y_crop, height_crop, width_x_crop, width_y_crop = (
            centroid(
                data_crop,
        ...
        return (
            max_x - shift + center_x_crop,
            max_y - shift + center_y_crop,
            height_crop,
            width_x_crop,
            width_y_crop,
        )
    ...
    losses, training_params = training_loop(
        m,
        y,
        opt,
        balance_weight=balance_weight,
    ...
    params = (
        center_x.detach().numpy(),
        center_y.detach().numpy(),
        height.detach().numpy(),
        np.abs(width_x.detach().numpy()),
        np.abs(width_y.detach().numpy()),
        angle.detach().numpy(),
    )
...
def discrete_width(data, n_grid=7):
    ...
    for width_x in np.arange(1, (n_grid + 1) * 0.5, 0.5):
        for width_y in np.arange(1, (n_grid + 1) * 0.5, 0.5):
            for angle in np.linspace(
                0, np.pi, n_grid
            ):  # Searching over angles from 0 to pi
                ...
                rotated_gaussian = lambda x, y: gaussian(
                    x, y, center_x, center_y, width_x, width_y, height, angle
                )
...
def compute_lsn_design_matrix(lsn):
    ...
    """
    stim_pixels = lsn.frame_images[lsn.stim_table.frame.values].reshape(
        (-1, lsn.n_pixels)
    )
...
def calc_lsn_p_vals_for_col_vol_plane(
    ...
    lsn_vals, tot_lsn_frames, cell_indices = calc_lsn_vals(
        lsn,
        session,
        plane,
        onset_delay,
    ...
    if tot_lsn_frames > 0:
        p_vals, _ = calc_pvals_for_plane_using_zscore_one_tailed(
            lsn,
            session,
            plane,
            lsn_vals,
...
def calc_ns12_p_vals_for_col_vol_plane(
    session: OPhysSession,
    plane: int,
    sample_spontaneous: int = 1000,
    onset_delay: float = -1,
    ...
    ns_vals, tot_lsn_frames = calc_ns12_vals(
        ns, session, plane, onset_delay, offset_delay, trace_type=trace_type
    )
    all_ns_vals.append(ns_vals)

    if tot_lsn_frames > 0:
        ns_p_vals, _ = calc_ns12_pvals_for_plane_using_zscore_one_tailed(
            ns,
            session,
            plane,
            ns_vals,
    ...
    ns_means = np.array(
        [
            ns_vals[stim_table_df[stim_table_df["image"] == i].index].mean(axis=0)
...
def calc_ns118_p_vals_for_col_vol_plane(
    session: OPhysSession,
    plane: int,
    sample_spontaneous: int = 1000,
    onset_delay: float = -1,
    ...
    ns_vals, tot_lsn_frames = calc_ns118_vals(
        ns, session, plane, onset_delay, offset_delay, trace_type=trace_type
    )
    all_ns_vals.append(ns_vals)

    if tot_lsn_frames > 0:
        ns_p_vals, _ = calc_ns118_pvals_for_plane_using_zscore_one_tailed(
            ns,
            session,
            plane,
            ns_vals,
    ...
    ns_means = np.array(
        [
            ns_vals[stim_table_df[stim_table_df["image_index"] == i].index].mean(axis=0)
            for i in range(n_images)
        ]
    )
    all_pref_images = np.argmax(ns_means, axis=0)
...
def get_plane_lsn_constants(session: OPhysSession, trace_type: str = "events"):
    """
...
def compute_lsn_ns_metrics_for_col_vol_plane(
    client: OPhysClient,
    mouse_id: str,
    col_vol_id: str,
    plane: int,
    ...
    all_ns12_vals_in_colvol_plane, all_ns12_pvals_in_colvol_plane, ns12_pref_images = (
        calc_ns12_p_vals_for_col_vol_plane(
            session,
            plane,
            sample_spontaneous,
            onset_delay,
    ...
    (
        all_ns118_vals_in_colvol_plane,
        all_ns118_pvals_in_colvol_plane,
        ns118_pref_images,
    ) = calc_ns118_p_vals_for_col_vol_plane(
        session,
        plane,
        sample_spontaneous,
        onset_delay,
    ...
    all_lsn_vals_in_colvol_plane, all_pvals_in_colvol_plane, cell_indices = (
        calc_lsn_p_vals_for_col_vol_plane(
            session,
            plane,
            sample_spontaneous,
            onset_delay,
    ...
    design_matrix, trial_template, frame_images = get_plane_lsn_constants(
        session, trace_type=trace_type
    )
    ...
    if lsn.n_rois_valid > 1:
        ...
        percentage_res_trial_4_locally_sparse_noise = np.zeros(
            n_valid_cells_in_colvol_plane, dtype=float
        )
        frac_res_trial_4_locally_sparse_noise = np.zeros(
            n_valid_cells_in_colvol_plane, dtype=float
        )
        ...
        n_responsive_trials = np.zeros(
            (n_valid_cells_in_colvol_plane, number_of_tot_pixels), dtype=int
        )
        weighted_avg = np.zeros((n_valid_cells_in_colvol_plane, number_of_tot_pixels))
        weighted_avg_only_resp_trials = np.zeros(
            (n_valid_cells_in_colvol_plane, number_of_tot_pixels)
        )
        ...
        n_responsive_trials_on = np.zeros(
            (n_valid_cells_in_colvol_plane, s1, s2), dtype=int
        )
        weighted_avg_on = np.zeros((n_valid_cells_in_colvol_plane, s1, s2))
        weighted_avg_only_resp_trials_on = np.zeros(
            (n_valid_cells_in_colvol_plane, s1, s2)
        )
        ...
        n_responsive_trials_off = np.zeros(
            (n_valid_cells_in_colvol_plane, s1, s2), dtype=int
        )
        weighted_avg_off = np.zeros((n_valid_cells_in_colvol_plane, s1, s2))
        weighted_avg_only_resp_trials_off = np.zeros(
            (n_valid_cells_in_colvol_plane, s1, s2)
        )
        ...
        ns12_pvals_means = np.array(
            [
                all_ns12_pvals_in_colvol_plane[
                    stim_table_ns12_df[
                        stim_table_ns12_df["image"] == ns12_pref_images[idx]
                    ].index,
                    idx,
                ]
                for idx in range(n_valid_cells_in_colvol_plane)
            ]
        )

        ns118_pvals_means = np.array(
            [
                all_ns118_pvals_in_colvol_plane[
                    stim_table_ns118_df[
                        stim_table_ns118_df["image_index"] == ns118_pref_images[idx]
                    ].index,
                    idx,
                ]
                for idx in range(n_valid_cells_in_colvol_plane)
        ...
        for cell in range(n_valid_cells_in_colvol_plane):
            ...
            frac_res_to_ns12[cell] = (
                ns12_pvals_means[cell, :] < response_thresh_alpha
            ).sum() / n_repeats_ns12
            frac_res_to_ns118[cell] = (
                ns118_pvals_means[cell, :] < response_thresh_alpha
            ).sum() / n_repeats_ns118
