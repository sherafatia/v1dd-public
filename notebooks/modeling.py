...
class ConvNet(nn.Module):
    def __init__(self, in_channels=1, height=32, width=32):
        ...
        )  # outputs (batch, 32, height, width)
        self.skip = nn.Identity()  # Placeholder for skip connection
        # self.conv3 = nn.Conv2d(
        #     48, 64, kernel_size=3, padding=1
        # )  # expects concatenation of conv1 and conv2 outputs
...
def add_noise_to_last_layer(model, noise_level=0.5):
    ...
    with torch.no_grad():
        noise = (
            torch.randn_like(model.conv3.weight.data) * noise_level
        )  # returns tensor filled w/ random numbers from standard normal distribution * noise_level
...
def train_model(
    model, train_loader, gabor_val_loader=None, cifar_val_loader=None, num_epochs=10
):
    model.to(device)
